...
public class ParkingVistaController implements Initializable {
    ...
    private Slider sldLargo;
    @FXML
    private Menu mnDatos;
    @FXML
    private MenuItem mniCargar;
    @FXML
    private MenuItem mniGuardar;
    ...
    private void modificarPlazas(ActionEvent event) {
        ...
        String fichero = Main.getFichero();
        ...
        if (!esNumero(this.txtNumeroDePlaza.getText())) {
            ...
            System.out.println("Ancho = " + this.txtAncho.getText());
            error = "Ancho";
            validacion = false;
        } else if (!esNumero(this.txtAncho.getText())) {
            error = "Largo";
    ...
    public Plaza buscarPlaza(int p) {
        Plaza plaza = null;
        ArrayList<Plaza> listaPlazas = Main.getListaPlazas();
    ...
    private void mostrarPlaza(Plaza p) {
        if (p != null) {
            ...
            this.txtAncho.setText((int) p.getAncho() + "");
            this.txtLargo.setText((int) p.getLargo() + "");
    ...
    private void limpiar() {
        ...
        this.rdbPagoAlDia_Si.setSelected(false);
        this.rdbPagoAlDia_No.setSelected(false);
        this.txtAncho.setText("0");
        this.txtLargo.setText("0");
    ...
    @FXML
    private void cargarFichero(ActionEvent event) {
        Stage stage = new Stage();

        FileChooser fileChooser = new FileChooser();
        fileChooser.setTitle("Abrir Fichero desde el que Cargar Datos");
        ...
        if (!file.getName().equals("")) {
            ...
            Main.setFichero(fichero);
            ArrayList<Plaza> listaPlazas = new ArrayList<>();
            listaPlazas = serializaFicheroInput(listaPlazas, fichero);
            Main.setListaPlazas(listaPlazas);
        }
        System.out.println(file.getName());
        Alert alert = new Alert(Alert.AlertType.INFORMATION);
        alert.setHeaderText(null);
        alert.setTitle("Carga realizada");
        alert.setContentText("Se ha realizado la carga de datos desde fichero. Los datos se han actualizado");
        alert.showAndWait();

    }

    @FXML
    private void guardarFichero(ActionEvent event) {

        Stage stage = new Stage();

        FileChooser fileChooser = new FileChooser();
        fileChooser.setTitle("Abrir Fichero para Guardar Datos");
        File file = fileChooser.showOpenDialog(stage);

        if (!file.getName().equals("")) {
            //openFile(file);
            Main.setFichero(file.getName());
            ArrayList<Plaza> listaPlazas = Main.getListaPlazas();
            serializaFicheroOutput(listaPlazas, file.getName());
        }
        System.out.println(file.getName());
        Alert alert = new Alert(Alert.AlertType.INFORMATION);
        alert.setHeaderText(null);
        alert.setTitle("Guardado de datos realizado");
        alert.setContentText("Se ha realizado el guardado de datos en fichero. Los datos se han actualizado");
        alert.showAndWait();
